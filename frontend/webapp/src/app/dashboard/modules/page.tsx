...
export default function ModulesPage() {
  ...
  const onSubmit = async (data: ModuleFormValues) => {
    try {
      ...
      setError(e.response?.data?.detail || 'Error creando módulo');
    }
  };
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-[var(--color-outline-variant)]">
            {loading ? (
              ...
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-[var(--color-on-surface-variant)]">No se encontraron módulos</td>
              </tr>
            ) : (
              modules.map(mod => (
                <tr key={mod.id} className="hover:bg-[var(--color-surface-container-lowest)] transition-colors">
                  <td className="px-6 py-4">
                    ...
                    <div className="text-mono-label text-[var(--color-on-surface-variant)] mt-1 text-xs">{mod.id}</div>
                  </td>
                  <td className="px-6 py-4 text-body-md text-[var(--color-on-surface)]">{mod.descripcion}</td>
                  <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-elevation-2 w-full max-w-md border border-[var(--color-outline-variant)]">
            <div className="px-6 py-4 border-b border-[var(--color-outline-variant)] flex justify-between items-center">
              <h2 className="text-headline-sm text-[var(--color-on-surface)]">Crear Nuevo Módulo</h2>
